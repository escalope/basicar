...
import es.ucm.look.ar.LookVisual;
...
public class AR2D extends View {
	...
	private LookVisual contextVisual;
	 ...
	 */
	public AR2D(World world, LookVisual context, float maxDist) {
		super(context);
	...
	protected void onDraw(Canvas canvas) {
		...
		contextVisual.getWorld().update(elapsed);
	...
	private void projectEntities(Canvas canvas, long elapsed) {
		synchronized (listLock) {			
			...
			for (WorldEntity we : contextVisual.getWorld().getWorldEntities()) {
				if (we.isVisible()) {
					Point3 pr = new Point3(we.getLocation());
					pr.add(-contextVisual.getWorld().getLocation().x, -contextVisual.getWorld().getLocation().y,
							-contextVisual.getWorld().getLocation().z);
	...
	public void projectPoint(Point3 p) {
		...
		p.add(-contextVisual.getWorld().getLocation().x, -contextVisual.getWorld().getLocation().y,
				-contextVisual.getWorld().getLocation().z);
	...
	private WorldEntity getEntity(int x, int y) {
		...
		if (buffer != null) {
			...
			if (x > 0 && x < buffer.getWidth() && y > 0
				...
				return contextVisual.getWorld().getWorldEntity(id);
			}
		}
		return null;
	}
